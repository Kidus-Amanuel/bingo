...
export default function DepositsPage() {
    ...
    if (loading) return (
        <div className="space-y-6 pt-12 text-center">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Loading Deposit Ledger...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400">
                ...
                <span className="text-slate-900">Deposits</span>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <h1 className="text-xl font-bold text-slate-900 tracking-tight">Deposit Audit</h1>
                    </div>
                    <p className="text-sm text-slate-500 font-medium">Verify incoming liquidity and player top-ups.</p>
                </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center">
                <div className="relative flex-1 group w-full">
                    ...
                    <input 
                        type="text" 
                        placeholder="Search by username or TXID..." 
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="h-10 pl-9 pr-4 bg-white border border-slate-200 rounded-lg text-sm font-medium w-full outline-none focus:ring-1 focus:ring-indigo-500 transition-all shadow-sm"
                    />
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                    <select 
                        ...
                        className="h-10 px-3 bg-white border border-slate-200 rounded-lg text-xs font-bold uppercase tracking-wider outline-none focus:ring-1 focus:ring-indigo-500 shadow-sm"
                    >
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-100">
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">Processed At</th>
                            </tr>
                        </thead>
                    ...
                    {filteredDeposits.length === 0 && (
                        <div className="py-24 text-center">
                            <ArrowDownToLine size={48} className="text-slate-100 mx-auto mb-4" />
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest">No Deposits Found</h3>
                            <p className="text-xs text-slate-400 mt-1">Refine your filters or search query.</p>
                        </div>
                    )}
                </div>
            </Card>
